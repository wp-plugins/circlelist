...
class CircleList_Shortcode {
	...
	function shortcode_circleList( $attr ) {
		...
		$args = array(
			...
			'posts_per_page' => -1,
			'circlelist_taxonomy' => $attr['category'],
			'meta_key' => $attr['sort'],
			'orderby' => 'meta_value',
			'order'	=> 'ASC',
		);
	...
	function shortcode_circleInfo( $attr ) {
		...
		echo '<dl>';
		printf('<dt>%s</dt><dd>%s</dd>', esc_html( __('Circle Name', CircleList::TEXTDOMAIN) ), $circleData['CircleName'][0] );
		printf('<dt>%s</dt><dd>%s</dd>', esc_html( __('Circle Kana', CircleList::TEXTDOMAIN) ), $circleData['CircleKana'][0] );
		printf('<dt>%s</dt><dd>%s</dd>', esc_html( __('Author Name', CircleList::TEXTDOMAIN) ), $circleData['AuthorName'][0] );
		printf('<dt>%s</dt><dd>%s</dd>', esc_html( __('Author Kana', CircleList::TEXTDOMAIN) ), $circleData['AuthorKana'][0] );
		...
		$taxonomy = get_the_terms( $post->ID, 'circlelist_taxonomy' );
